---
title: "Grounded autonomous scrutiny at scale"
type: source
...
Relevance: a contemporary, corpus-grounded autonomous research agent — the read-plan-compute-compare loop over a fixed document corpus is the [[Auto-Researcher]] pattern made concrete, extending STORM's article writing ([[STORM: Writing Wikipedia-like Articles From Scratch]]) by grounding in executable ground truth. By contrast with an auto-researcher whose only action is reading an author's notes, its headline result locates the value in *executing and comparing*, not reading.
